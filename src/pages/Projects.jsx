  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingProject) {
        ...
        if (formStatus !== editingProject.status) {
          // FIX M4: owners and admins can both change status; backend enforces ownership
          await updateProjectStatus(editingProject.id, formStatus);
        }
        toast.success("Project updated!");
      } else {
        await createProject({ name: formName, description: formDesc });
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              {editingProject && (
                <div>
                  ...
                  {/* FIX M4: show to any editor; backend guards ownership */}
                  <select
                    className="border border-gray-300 rounded-lg w-full px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
